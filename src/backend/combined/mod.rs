...
where AudioInErr: Display, AudioOutErr: Display
{
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        match self {
            CombinedError::AudioInError(ref e) => {
                write!(f, "Audio in error: {}", e)
            }
            CombinedError::AudioOutError(ref e) => {
                write!(f, "Audio out error: {}", e)
            }
        }
    }
}
...
where AudioInErr: Error, AudioOutErr: Error,
{
